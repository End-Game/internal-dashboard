	...
	$.ajax(
		...
		success:function(data){
			...
			for(var i=0; i < deadlines.length; i++){
				...
				div = '<div><p class="date_text">' + deadline.date + '</p><p class="text">'+
					deadline.task +'</p></div>';
			...
			for(i=0; i < newsFeed.length; i++){
				var news = newsFeed[i];
				div = '<div><p class="date_text">' + news.date + '</p><p class="text">' +
			...
			for(i=0; i < usefulLinks.length; i++){
				...
				div = '<div><p class="text"><a href="' + link.link + '">' + link.name +
